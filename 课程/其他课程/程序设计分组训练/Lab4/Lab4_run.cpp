...
void run(int argc, char *argv[])
{
	...
	bool user_def_dir = false; //是否用户自定义路径
	...
	while (--argc > 0) 
	{
		if (!illegal_number(*++argv) || strcmp(*argv, "r") == 0) //如果是合法数字，或随机处理
		{
			if (strcmp(*argv, "r") == 0)
			{
				configinfo.number = rand_num(configinfo.recordcount2, configinfo.recordcount1); //随机生成
			}
			else
			{
		...
		else if (strcmp(*argv, "t") == 0 || strcmp(*argv, "d") == 0) //输出文件格式
		{
			if (strcmp(*argv, "d") == 0) //二进制文件
			{
	...
	{
		separate_filepath(filepath); //分离路径及文件名，存入配置
	}
