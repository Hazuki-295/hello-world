...
int main()
{
	...
	while (getline(cin, line)) // 处理一行
	{
		...
		while (buffer >> num1 >> num2) // 模拟加法
		{
			...
			{
				bool copy1_copy2 = true;              // 假设copy1>copy2成立

				if (copy1.size() < copy2.size())
				{
					copy1_copy2 = false;
				}
				else if (copy1.size() == copy2.size()) // 等长
				{
					if (copy1 == copy2) copy1_copy2 = false;
					auto p1 = copy1.begin(), p2 = copy2.begin();
					while (p1 != copy1.end() && p2 != copy2.end())      // 未到达结尾
						if (*p1 < *p2) { copy1_copy2 = false; break; }
						else if (*p1 == *p2) { p1++; p2++; }
						else if (*p1 > *p2) { break; }
				}
